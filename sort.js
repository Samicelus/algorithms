class Sort{
    ...
    quickMergeSort(arr, path){
        if(arr.length <= 1){
            return arr;
        }
        let pivotIndex = Math.floor(arr.length /2);
        let pivot = path?getEmbeddedValue(arr[pivotIndex], path):arr[pivotIndex];
        let left = [];
        let right = [];
        for(let i = 0; i< arr.length; i ++){
            if(path?getEmbeddedValue(arr[i], path):arr[i] < pivot){
                left.push(arr[i]);
            }else if(i != pivotIndex){
                right.push(arr[i]);
            }
        }
        return this.quickMergeSort(left, path).concat([pivot], this.quickMergeSort(right, path));
    }
